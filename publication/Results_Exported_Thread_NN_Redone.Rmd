---
title: "Publication - Results, Thread Fix, NN Redone Twice"
author: "CAKU"
date: "`r Sys.Date()`"
...
results = do.call("rbind", lapply(results_paths,function(path) {
  ...
  dplyr::mutate(path=recode(path,
                            "siteNN/ResultsNNThread.zip"="NN_FTP_Mark1",
                            "siteNN/ResultsNN_FTP_MARK2.zip"="NN_FTP_Mark2")) %>% 
  separate(path,into=c("site","source","mark")) %>% 
  filter(site=="NN") %>%  # only use NN FTP
...
p1a =
  ...
  group_by( target_id , comparator_id , outcome_id , analysis_id , source, site , stat ) %>% 
  ...
  labs(title="Fig X: Cohort Counts and Days with Zero / Above SD",
       subtitle="Site 3: Two Docker-based Result Analysis Runs",
       x=NULL,
       y="Count") +
  facet_wrap(~source)
...
ggsave(filename = "~/EFPIA-RWD-SUBMISSION-PILOT/publication/p1a_thread_redone.png",plot = p1a,device = "png",width = 7,height=7)
...
ggsave(filename = "~/EFPIA-RWD-SUBMISSION-PILOT/publication/p2a_thread_redone.png",plot = p2a,device = "png",width = 7,height=7)
...
p2b = 
  ...
  filter(stat == "se_log_rr") %>% 
  group_by( target_id , comparator_id , outcome_id , analysis_id , stat , source ) %>% 
  arrange( target_id , comparator_id , outcome_id , analysis_id , stat , source, mark )%>% 
  dplyr::mutate(sd_=sd(val),
                mean_=mean(val)) %>% 
  dplyr::mutate(lwr=mean_-qnorm(p = 0.975)*sd_,
                upr=mean_+qnorm(p = 0.975)*sd_) %>%
...
ggsave(filename = "~/EFPIA-RWD-SUBMISSION-PILOT/publication/p2b_thread_redone.png",plot = p2b,device = "png",width = 7,height=7)
...
p3a = results_pm %>% 
  dplyr::select( analysis_id , target_id , comparator_id , covariate_id  , source , site, mark , coefficient) %>% 
  arrange( analysis_id , target_id , comparator_id , covariate_id  , source , site, mark ) %>% 
  distinct() %>% 
...
ggsave(filename = "~/EFPIA-RWD-SUBMISSION-PILOT/publication/p3a_thread_redone.png",plot = p3a,device = "png",width = 7,height=7)
...
results_cb_sd <- results_pm %>%
  ...
  arrange(analysis_id,outcome_id,target_id,comparator_id,covariate_id,stat,source,mark) %>% 
  summarise(sd = sd(val)) %>%
...
ggsave(filename = "~/EFPIA-RWD-SUBMISSION-PILOT/publication/p4a_thread_redone.png",plot = p4a, device = "png",width = 8,height=8)
