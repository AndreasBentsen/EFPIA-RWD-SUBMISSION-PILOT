...
ggsave(filename = "p3.png",plot = p3,device = "png",width = 8,height=8)
...
results_cb_sd <- results_pm %>%
  ...
  summarise(sd = sd(val)) %>%
  group_by(analysis_id,outcome_id,target_id,comparator_id,covariate_id) %>% 
  dplyr::mutate(max_sd=max(sd)) %>% 
  unite(analysis_id,analysis_id,outcome_id,target_id,comparator_id,covariate_id) %>%
  dplyr::mutate(n_sd=length(unique(analysis_id))) %>% 
  filter(sd > 0) %>% 
  dplyr::mutate(n_sd_above_0=length(unique(analysis_id))) %>% 
  arrange(desc(max_sd)) %>%  
  dplyr::mutate(analysis_id=factor(analysis_id,levels=rev(unique(.$analysis_id))))
# Visualize
...
p5 = results_cb_sd %>%
  # sample_n(1000) %>% 
  ggplot(.,aes(x=analysis_id,y=sd,shape=stat)) +
  geom_point(alpha=0.3) +
  coord_flip() +
...
ggsave(filename = "p5.png",plot = p5, device = "png",width = 8,height=8)
